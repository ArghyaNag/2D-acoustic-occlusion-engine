...
class SharedState:
    ...
    def get_snapshot(self) -> dict:
        with self._lock:
            return {'listener_pos': self._listener_pos, 'sources': copy.deepcopy(self._sources), 'walls': frozenset(self._walls)}
    ...
    def add_source(self, pos: tuple[int, int]) -> int:
        with self._lock:
            ...
            self._sources[sid] = {'pos': pos, 'audio_path': None, 'loop': True, 'playing': False, 'input_mode': 'file', 'network_client': None}
            return sid
    ...
    def set_source_network_client(self, source_id: int, client_key: tuple[str, int] | None) -> None:
        with self._lock:
            if source_id in self._sources:
